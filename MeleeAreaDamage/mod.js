...
[
  ...
  'global\\excel\\base\\properties.txt',
].forEach((fileName) => {
  const fileContent = D2RMM.readTsv(fileName);
  if (!fileContent) return;
  fileContent.rows.push({
...
if (config.scha || config.mcha || config.lcha) {
  [
    ...
    const group = Math.max(...fileContent.rows.map((row) => row.group)) + 1;
...
['global\\excel\\skills.txt', 'global\\excel\\base\\skills.txt'].forEach(
  (fileName) => {
    const fileContent = D2RMM.readTsv(fileName);
    if (!fileContent) return;
    fileContent.rows.push({
      skill: 'Melee Area Damage', // links with missiles.txt
      '*Id': Math.max(...fileContent.rows.map((row) => row['*Id'])) + 1,
      ...
      SrcDam: Math.max(
        1,
        Math.min(128, Math.round((128 * damageInArea) / 100)),
      ), // percentage of weapon damage applied to skill (base 128)
...
['global\\excel\\missiles.txt', 'global\\excel\\base\\missiles.txt'].forEach(
  (fileName) => {
    const fileContent = D2RMM.readTsv(fileName);
    if (!fileContent) return;
    fileContent.rows.push({
...

{
  ...
  fileContent.meleeareadamage = 'melee_area_damage';
...
if (config.unique) {
  ['global\\excel\\misc.txt', 'global\\excel\\base\\misc.txt'].forEach(
    (fileName) => {
      ...
      if (!fileContent) return;
      fileContent.rows.push({
        ...fileContent.rows.find((row) => row.name === 'Small Charm'),
  ...
  [
    ...
    fileContent.rows.push({
      ...fileContent.rows.find((row) => row.index === 'Annihilus'),
